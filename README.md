...
- `main`: reviewed production release for `www.ltl.co.jp`; deployment evidence
  and rollback details are retained in `docs/PRODUCTION_DEPLOYMENT.md`.
...
- `plugin-patches/`: compatibility and security patches retained for future
  plugin upgrades.
- `mu-plugins/`: host-limited safeguards used only on the `test` branch.
